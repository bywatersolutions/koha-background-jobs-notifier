...
my ( $opt, $usage ) = describe_options(
    ...
    [   "state-file|f=s",
        ...
        {   default => $ENV{XDG_DATA_HOME}
            ? "$ENV{XDG_DATA_HOME}/koha_job_monitor_state.json"
            : "$ENV{HOME}/.local/share/koha_job_monitor_state.json"
        }
    ],
...
# Count of new jobs
my ($new_count) = $dbh->selectrow_array("SELECT COUNT(*) FROM background_jobs WHERE status='new' AND queue='$QUEUE'");

# Rate of new jobs
my ($jobs_rate) = $dbh->selectrow_array(
    qq{
    SELECT COUNT(*)
...
# Summary per type
my $summary_rows = $dbh->selectall_arrayref(
    qq{
...
sub print_summary_table {
    ...
    # Print header
    printf "%-*s  %-*s  %*s\n", $w_type, "type", $w_status, "status", $w_count, "count";
    ...
    # Print rows
    foreach my $r (@$rows) {
        printf "%-*s  %-*s  %*d\n", $w_type, $r->{type}, $w_status, $r->{status}, $w_count, $r->{c};
    }
}
